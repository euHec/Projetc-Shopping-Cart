...
export const removePrice = async (id) => {
  ...
  console.log(price);
  total -= price;
  if (total < 0) totalPrice.innerHTML = 0;
  console.log(total);
  totalPrice.innerHTML = total.toFixed(2);
  return total;
};

export const addPrice = async (id) => {
  const { price } = await fetchProduct(id);
  console.log(price);
  total += price;
  console.log(total);
...
export const inputCart = async ({ target }) => {
  const ol = document.querySelector('.cart__products');
  const elementID = getIdFromProduct(target.parentNode);
  saveCartID(elementID);
  addPrice(elementID);
  const returned = await fetchProduct(elementID);
  const li = createCartProductElement(returned);
  ol.appendChild(li);
};
...
export const createProductElement = ({ id, title, thumbnail, price }) => {
  ...
  cartButton.addEventListener('click', (e) => inputCart(e));
